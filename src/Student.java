...
public class Student extends Person {
    ...
    public Student(int Id, String Name, Date Dob, String Contact, String password, int enrollment) {
    ...
    *   Check if the administrator ID is valid:
            ...
            The first five digits should be 84023.
    ...
    * */
    ...
    public void setEnrollment(int enrollment) {
        if (enrollment <= 0) {
            throw new IllegalArgumentException("Enrollment number must be greater than 0");
        }
        this.enrollment = enrollment;
    }
    ...
    protected boolean validate() {
        if (enrollment <= 0) {
            OutputFormatter.errorText("Invalid enrollment number", "Enrollment number must be greater than 0");
            throw new IllegalArgumentException("Enrollment number must be greater than 0");
        }
        String idStr = String.valueOf(getId());
        if (idStr.length() != 9 || !idStr.startsWith("84023")) {
            OutputFormatter.errorText("Invalid student ID", "Student ID must start with 84023 and be 9 digits long");
            throw new IllegalArgumentException("Invalid student ID");
        }
        if (getName() == null || getName().isEmpty()) {
            OutputFormatter.errorText("Invalid name", "Name cannot be null or empty");
